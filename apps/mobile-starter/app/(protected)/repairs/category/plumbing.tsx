...
export default function PlumbingRepairsScreen() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['repairs', estateId, 'mine', 'plumbing'],
    queryFn: async () => {
      if (!estateId) return [];
      const res = await fetch(`/api/estates/${estateId}/repairs?scope=mine&category=plumbing`);
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <Pressable style={[s.row, !isLast && s.rowBorder]} onPress={() => router.push(`/repairs/${item.id}` as never)}>
        <View style={s.catBox}><Ionicons name="water-outline" size={22} color="#0ea5e9" /></View>
        <View style={{ flex: 1 }}>
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.header}>
        ...
        <Text style={s.hTitle}>Plumbing Repairs</Text>
        <View style={{ width: 38 }} />
      </View>
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              <Ionicons name="water-outline" size={48} color={colors.neutral.placeholder} />
              <Text style={s.emptyTxt}>No plumbing repair tickets</Text>
            </View>
          }
        />
      )}
...
const s = StyleSheet.create({
  ...
  catBox: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: '#0ea5e922' },
