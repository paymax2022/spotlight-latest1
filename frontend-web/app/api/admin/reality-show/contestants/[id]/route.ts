...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'promote_to_bootcamp') {
      return successResponse({ contestant: await promoteToBootcamp(params.id) });
    }
    if (action === 'fail_audition') {
      return successResponse({ contestant: await failAudition(params.id) });
    }
    if (action === 'declare_winner') {
      return successResponse({ contestant: await updateContestant(params.id, { phaseStatus: 'winner', finalistPosition: 1, isActive: false }) });
    }
    if (action === 'declare_finalist') {
      return successResponse({ contestant: await updateContestant(params.id, { phaseStatus: 'finalist', finalistPosition: Number(body.position ?? 2) }) });
    }
    ...
    return successResponse({ contestant: await updateContestant(params.id, patch as Parameters<typeof updateContestant>[1]) });
  } catch (error) {
