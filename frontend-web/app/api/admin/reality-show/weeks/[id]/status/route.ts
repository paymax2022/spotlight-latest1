...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (status === 'open') return successResponse({ week: openVoting(params.id) });
    if (status === 'closed') return successResponse({ week: closeVoting(params.id) });
    if (status === 'upcoming') return successResponse({ week: updateWeek(params.id, { status: 'upcoming' }) });
    ...
    return successResponse({ week: updateWeek(params.id, patch as Parameters<typeof updateWeek>[1]) });
  } catch (error) {
