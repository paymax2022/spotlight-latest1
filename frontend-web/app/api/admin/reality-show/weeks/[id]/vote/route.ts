...
import { getWeek, castVote, retractVote, getVotesForWeek, getVoteTallies, getContestant } from '@/src/server/services/reality-show/persistence';
...
export async function GET(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const votes = await getVotesForWeek(params.id);
    // Each tally needs its contestant. Promise.all so the lookups overlap —
    // sequential awaits here would be one round-trip per contestant.
    const tallies = await Promise.all(
      (await getVoteTallies(params.id)).map(async (t) => ({
        ...t,
        contestant: await getContestant(t.contestantId),
      })),
    );
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.retract) {
      await retractVote(params.id, identity.actorId, body.contestantId);
