...
/**
 * Catch-all proxy: /api/v1/<...> → Go /api/v1/<...>.
 *
 ...
 *
 * Specific routes still win: Next matches app/api/v1/kyc/me/route.ts before this
...
async function forward(request: Request, ctx: { params: Promise<{ path: string[] }> }) {
  ...
  return proxyToGoBackend(request, `/api/v1/${path.join('/')}`);
}
