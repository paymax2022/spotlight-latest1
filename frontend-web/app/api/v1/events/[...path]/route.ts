...
// Catch-all proxy: /api/v1/events/<...> → Go /api/finance/events/<...>
...
async function forward(request: Request, path: string[]) {
  if (!featureFlags.events()) return errorResponse('This service is not available.', 503);
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/finance/events/${sub}`);
