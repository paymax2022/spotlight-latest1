...
// Catch-all proxy: /api/v1/savings/<...> → Go /api/finance/savings/<...>
...
async function forward(request: Request, path: string[]) {
  if (!featureFlags.savings()) return errorResponse('This service is not available.', 503);
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/finance/savings/${sub}`);
