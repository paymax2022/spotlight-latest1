...
// Catch-all proxy: /api/v1/placement/<...> → Go /api/finance/placement/<...>.
// Covers campaigns/*, zones, and events. Authed (Go enforces flags/authZ/RBAC).
async function forward(request: Request, path: string[]) {
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/finance/placement/${sub}`);
