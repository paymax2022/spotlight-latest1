...
// Catch-all proxy: /api/v1/invest/<...> → Go /api/v1/invest/<...>.
// No matching feature flag exists, so the flag check is intentionally omitted —
// Go enforces flags/authZ, eligibility, suitability and the ledger invariants.
// Money mutations forward the Idempotency-Key.
async function forward(request: Request, path: string[]) {
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/v1/invest/${sub}`);
