...
// Catch-all proxy: /api/v1/crypto/<...> → Go /api/v1/crypto/<...>.
// NOTE: at the time of writing the Go backend has NO /api/v1/crypto route group
// registered (the only crypto code lives in the standalone mobile-app backend).
// This proxy targets the canonical /api/v1/crypto prefix the mobile client calls
// so it is ready the moment the Go group lands; until then upstream returns 404.
// No matching feature flag exists, so the flag check is intentionally omitted —
// Go enforces flags/authZ. Money mutations forward the Idempotency-Key.
async function forward(request: Request, path: string[]) {
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/v1/crypto/${sub}`);
