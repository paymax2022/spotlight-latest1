...
// Catch-all proxy: /api/v1/creators/<...> → Go /api/finance/creators/<...>
...
async function forward(request: Request, path: string[]) {
  if (!featureFlags.creators()) return errorResponse('This service is not available.', 503);
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/finance/creators/${sub}`);
