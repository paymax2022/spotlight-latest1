...
// Catch-all proxy: /api/v1/academy/<...> → Go /api/finance/academy/<...>.
// No matching feature flag exists, so the flag check is intentionally omitted —
// Go enforces flags/authZ and the ledger invariants. Money mutations forward the
// Idempotency-Key.
async function forward(request: Request, path: string[]) {
  try {
    await requireRequestUser(request);
    const sub = (path ?? []).join('/');
    return proxyToGoBackend(request, `/api/finance/academy/${sub}`);
