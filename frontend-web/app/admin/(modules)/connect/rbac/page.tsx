...
export default function ConnectRbacPage() {
  ...
  return (
    <Page>
      ...
      {loading ? (
        <Card><p style={{ color: colors.muted }}>Loading RBAC…</p></Card>
      ) : !data ? (
        <Card><p style={{ color: colors.muted }}>No RBAC data.</p></Card>
      ) : (
        <>
          <Card title={`Admin users (${data.admins.length})`}>
            {data.admins.length === 0 ? (
              <p style={{ color: colors.muted }}>No admins assigned.</p>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {data.admins.map((a) => (
                    <tr key={a.id}>
                      <td style={td()}><strong>{a.name}</strong><div style={{ color: colors.muted, fontSize: '0.78rem' }}>{a.email}</div></td>
                      <td style={td()}>{a.roles.map((r) => r.replace('connect.role.', '')).join(', ')}</td>
                      <td style={td()}>{a.mfa_enabled ? <span style={{ color: colors.success }}>Enabled</span> : <span style={{ color: colors.warning }}>Off</span>}</td>
          ...
          <Card title={`Roles (${data.roles.length})`}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {data.roles.map((r) => (
                  <tr key={r.id}>
                    <td style={td()}><strong>{r.name}</strong>{r.is_system ? <span style={{ marginLeft: 6, color: colors.muted, fontSize: '0.72rem' }}>system</span> : null}</td>
