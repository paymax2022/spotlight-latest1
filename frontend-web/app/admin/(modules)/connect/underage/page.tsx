...
export default function ConnectUnderagePage() {
  ...
  return (
    <Page>
      ...
      <Card>
        {loading ? <p style={{ color: colors.muted }}>Loading underage queue…</p> : rows.length === 0 ? (
          <p style={{ color: colors.muted }}>No underage flags in this state.</p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  <td style={td()}><Link href={`/admin/connect/users/${r.user_id}`} style={{ color: colors.info, textDecoration: 'none' }}>{r.handle}</Link></td>
                  ...
                  <td style={td()}>{r.case_id ? <Link href="/admin/connect/cases" style={{ color: colors.info, textDecoration: 'none' }}>{r.case_id}</Link> : <span style={{ color: colors.muted }}>—</span>}</td>
