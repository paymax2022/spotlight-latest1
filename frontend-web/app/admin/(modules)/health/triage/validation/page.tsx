...
function Bar({ label: lbl, value, color, danger }: { label: string; value: number; color: string; danger?: boolean }) {
  return (
    <div style={{ marginBottom: '0.55rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: colors.text, marginBottom: 2 }}>
        <span>{lbl}</span>
        <strong style={{ color: danger && value > 0.02 ? colors.danger : colors.text }}>{pct(value)}</strong>
      </div>
      <div style={{ height: 10, background: colors.headBg, borderRadius: 4, overflow: 'hidden' }}>
...
function ParityBlock({ title, p }: { title: string; p: LanguageParity }) {
  return (
    <div style={{ border: `1px solid ${colors.border}`, borderRadius: '0.5rem', padding: '0.85rem 1rem', background: colors.card }}>
      ...
      <Bar label="Emergency sensitivity" value={p.emergency_sensitivity} color={colors.success} />
      <Bar label="Level accuracy" value={p.level_accuracy} color={colors.info} />
      <Bar label="Over-triage (controlled)" value={p.over_triage} color={colors.warning} />
      <Bar label="Under-triage (must be ~0)" value={p.under_triage} color={colors.danger} danger />
    </div>
  );
}
    ...
    <div style={{ padding: '0.5rem 0.5rem 2rem' }}>
      ...
      {run ? (
        <>
          ...
          <div style={{ border: `2px solid ${colors.success}`, background: tint(colors.success, 0.08), borderRadius: '0.75rem', padding: '1.25rem 1.5rem', marginBottom: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: 0.4, color: colors.success, fontWeight: 700 }}>Emergency sensitivity — the metric that matters most (SC-3)</div>
              <div style={{ fontSize: '3rem', fontWeight: 800, color: colors.success, lineHeight: 1.05, marginTop: '0.25rem' }}>{pct(run.emergency_sensitivity)}</div>
              <div style={{ fontSize: '0.8rem', color: colors.text, marginTop: '0.25rem' }}>True emergencies correctly escalated · target near-100% (≈0 missed)</div>
            </div>
            <div style={{ textAlign: 'right', fontSize: '0.78rem', color: colors.muted }}>
          ...
          <Card title="Secondary metrics">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem' }}>
              <div>
                <Bar label="Over-triage (controlled over-referral — acceptable)" value={run.over_triage} color={colors.warning} />
                <Bar label="Under-triage (missed/under-referred — release blocker, must be ~0)" value={run.under_triage} color={colors.danger} danger />
                <Bar label="Level accuracy (exact 5-level agreement)" value={run.level_accuracy} color={colors.info} />
              </div>
            </div>
            <p style={{ fontSize: '0.78rem', color: colors.muted, marginBottom: 0 }}>{run.notes}</p>
          </Card>

          <Card title="Per-language parity (en vs pcm)">
            <p style={{ fontSize: '0.78rem', color: colors.muted, marginTop: 0 }}>Accuracy must hold across languages — vernacular must not degrade safety (SC-3).</p>
        ...
        <Card title="No run yet">
          <p style={{ color: colors.muted, margin: 0 }}>Click <strong>Run shadow eval</strong> to evaluate the engine + red-flag layer against the clinician-panel vignette set. The headline emergency-sensitivity figure appears here first.</p>
        </Card>
      )}
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {packs.map((p) => (
                  <tr key={p.code}>
                    <td style={td()}>{p.label} <code style={{ fontSize: '0.72rem', color: colors.muted }}>{p.code}</code></td>
                    <td style={td()}>{p.voice_supported ? <Badge status="active" label="Voice" /> : <span style={{ color: colors.muted }}>—</span>}</td>
                    <td style={td()}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        <div style={{ width: 90, height: 8, background: colors.headBg, borderRadius: 4, overflow: 'hidden' }}><div style={{ height: '100%', width: `${p.coverage_pct * 100}%`, background: colors.primary }} /></div>
                        <span style={{ fontSize: '0.74rem', color: colors.muted }}>{pct(p.coverage_pct)}</span>
                      </div>
                    </td>
                    <td style={td()}>{p.content_published}</td>
                    <td style={td()}>{p.content_pending > 0 ? <span style={{ color: colors.warning }}>{p.content_pending}</span> : 0}</td>
                ...
                {vignettes.map((v) => {
                  ...
                  return (
                    <tr key={v.id}>
                      ...
                      <td style={td()}>{v.last_eval_level ? <Badge status={v.last_eval_level} label={DISPOSITION_LABELS[v.last_eval_level]} /> : <span style={{ color: colors.muted }}>—</span>}</td>
                      <td style={td()}>{missedEmergency ? <span style={{ color: colors.danger, fontWeight: 700 }}>⚠ Missed emergency</span> : match ? <span style={{ color: colors.success }}>✓ Exact</span> : <span style={{ color: colors.warning }}>Off-level (safe)</span>}</td>
                    </tr>
                  );
                })}
              </tbody>
