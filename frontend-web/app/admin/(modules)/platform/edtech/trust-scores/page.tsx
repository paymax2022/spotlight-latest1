      ...
      <div style={{ padding: '0.5rem 0.5rem 2rem' }}>
        ...
        <StateBlock loading={loading} error={error} empty={false}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
            <Kpi label="Schools scored" value={rows.length.toString()} accent={colors.primary} />
            <Kpi label="Avg trust score" value={rows.length ? Math.round(rows.reduce((s, r) => s + r.score, 0) / rows.length).toString() : '—'} />
            <Kpi label="Overridden" value={rows.filter((r) => r.overridden).length.toString()} accent={colors.warning} />
          </div>

          {rows.map((r) => (
            <Card key={r.school_id} title={r.school_name} right={<span style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>{r.overridden ? <Badge status="warn" label="overridden" /> : null}<strong style={{ fontSize: '1.2rem', color: r.score >= 75 ? colors.success : r.score >= 50 ? colors.warning : colors.danger }}>{r.score}</strong></span>}>
              ...
              {r.overridden ? <div style={{ marginTop: '0.5rem', fontSize: '0.78rem', color: colors.warning }}>Override reason: {r.override_reason} · {fmtDate(r.updated_at)}</div> : null}
