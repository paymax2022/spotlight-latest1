      ...
      <div style={{ padding: '0.5rem 0.5rem 2rem' }}>
        ...
        <StateBlock loading={loading} error={error} empty={false}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
            <Kpi label="Active competitions" value={comps.filter((c) => c.status !== 'archived' && c.status !== 'completed').length.toString()} accent={colors.primary} />
            <Kpi label="In progress" value={comps.filter((c) => c.status === 'in_progress').length.toString()} accent={colors.success} />
          ...
          {comps.map((c) => {
            ...
            return (
              <Card key={c.id} title={c.name} right={<span style={{ display: 'flex', gap: '0.4rem' }}><Badge status={c.scope} label={c.scope} /><Badge status={c.status} label={c.status.replace(/_/g, ' ')} />{c.broadcast_ready ? <Badge status="ready" label="broadcast" /> : null}</span>}>
                <div style={{ fontSize: '0.82rem', color: colors.muted }}>{c.participating_schools} schools · {c.sponsor ? `sponsor: ${c.sponsor} · ` : ''}{fmtDate(c.start_date)} → {fmtDate(c.end_date)}</div>
                {nexts.length ? (
                ...
                ) : <div style={{ marginTop: '0.5rem', fontSize: '0.8rem', color: colors.muted }}>Terminal state — no further transition.</div>}
