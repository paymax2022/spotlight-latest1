  ...
  return (
    <Page style={{ padding: '0.5rem 0.5rem 2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
        {PROVIDERS.map((p) => (
          <div key={p} style={{ border: `1px solid ${colors.border}`, borderRadius: '0.5rem', padding: '1rem', background: colors.card }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
              <span style={{ width: 9, height: 9, borderRadius: 9999, background: colors.muted, display: 'inline-block' }} />
              <strong style={{ textTransform: 'capitalize' }}>{p.replace(/_/g, ' ')}</strong>
            </div>
            <div style={{ fontSize: '0.8rem', color: colors.muted }}>Success rate: —</div>
            <div style={{ fontSize: '0.8rem', color: colors.muted }}>p95 latency: —</div>
            <div style={{ fontSize: '0.8rem', color: colors.muted }}>Cost / check: —</div>
          </div>
        ))}
      </div>
      ...
      <Card title="Failover events">
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            <tr><td colSpan={4} style={{ ...td(), color: colors.muted }}>No failover events loaded.</td></tr>
          </tbody>
        </table>
      </Card>
    </Page>
