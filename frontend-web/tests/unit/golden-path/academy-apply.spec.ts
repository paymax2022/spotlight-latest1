...
describe('POST /api/academy/apply', () => {
  ...
  it('requires base fee PLUS every selected area, not the base alone', async () => {
    setupPaidMock(5000, [
      { slug: 'acting', fee_ngn: 2000 },
      { slug: 'editing', fee_ngn: 3000 },
    ]);
    // Pays only the base while selecting two priced areas: 5000 < 5000+2000+3000.
    paystackPaid(5000);

    const res = await POST(makeRequest('/api/academy/apply', {
      body: makeApplyBody({
        areas_of_interest: ['acting', 'editing'],
        application_fee_reference: 'ref-underpaid',
      }),
    }));
    const body = await res.json();

    expect(res.status).toBe(400);
    expect(body.error).toMatch(/lower than the required amount/i);
    expect(body.error).toContain('10,000');   // 5000 + 2000 + 3000
  });
  ...
  it('ignores any total the client claims', async () => {
    setupPaidMock(5000, [{ slug: 'acting', fee_ngn: 2000 }]);
    paystackPaid(5000);   // paid less than the real 7000
