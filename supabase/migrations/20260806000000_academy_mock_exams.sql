...
-- ───────────────────── Forward declarations (fresh-replay ordering) ─────────
-- This migration is timestamped BEFORE 20260815000800_academy_core.sql, which
-- defines the academy spine tables our FKs reference. On databases where the
-- chain already applied, core created these first and every statement below is
-- a no-op (IF NOT EXISTS). On a fresh replay, these create the tables early
-- with definitions IDENTICAL to academy_core so the final schema is the same
-- either way. Do not edit these independently of academy_core.
