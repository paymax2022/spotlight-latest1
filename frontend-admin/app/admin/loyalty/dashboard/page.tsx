...
import { PageHeader, LoyaltyTabs, Card, Kpi, Badge, DisclosureNote, StateBlock, btn, th, td, timeAgo, pct } from '../../events/_ui';
...
export default function LoyaltyDashboardPage() {
  ...
  return (
    <div style={{ padding: '0.5rem 0.5rem 2rem' }}>
      <PageHeader title="Loyalty overview" subtitle="Points liability (non-cash), tier distribution, earn/redeem trend and redemption fraud across all live modules." action={<button onClick={load} style={btn()}>Refresh</button>} />
        ...
        {data && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
              <Kpi label="Points outstanding" value={formatPoints(data.points_outstanding)} sub="Unredeemed (non-cash)" accent="#340075" />
              <Kpi label="Liability valuation" value={formatNaira(data.points_liability_kobo)} sub="NL-4 — valuation, not cash owed" accent="#9a3412" />
              <Kpi label="Redemption value" value={`${formatNaira(data.points_redemption_value_kobo)} / pt`} />
              <Kpi label="Earned (30d)" value={formatPoints(data.points_earned_30d)} accent="#15803d" />
              <Kpi label="Redeemed (30d)" value={formatPoints(data.points_redeemed_30d)} />
              <Kpi label="Expiring (30d)" value={formatPoints(data.points_expiring_30d)} accent={data.points_expiring_30d > 0 ? '#9a3412' : undefined} />
              ...
              <Kpi label="Redemption fraud open" value={data.redemption_fraud_open.toLocaleString('en-NG')} accent={data.redemption_fraud_open > 0 ? '#b91c1c' : undefined} />
            </div>

            <Card title="Tier distribution">
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr><th style={th()}>Tier</th><th style={th()}>Members</th><th style={th()}>Share</th></tr></thead>
                <tbody>
                  {data.tier_distribution.map((t) => (
                    <tr key={t.tier}>
                      <td style={td()}>{t.tier}</td>
                      <td style={td()}>{t.members.toLocaleString('en-NG')}</td>
                      <td style={td()}>{pct(t.share_pct)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <Card title="Points earned vs redeemed (14d)" right={<span style={{ fontSize: '0.75rem', color: '#6b7280' }}>Earned (purple) vs redeemed (green)</span>}>
              {data.points_trend.length === 0 ? <p style={{ color: '#6b7280' }}>No data.</p> : (
                      ...
                      <div key={p.date} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                            <div style={{ height: 10, width: `${eW}%`, minWidth: 2, background: '#340075', borderRadius: 2 }} title={`Earned ${formatPoints(p.earned)}`} />
                            <span style={{ fontSize: '0.7rem', color: '#6b7280', whiteSpace: 'nowrap' }}>{formatPoints(p.earned)}</span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                            <div style={{ height: 10, width: `${rW}%`, minWidth: 2, background: '#15803d', borderRadius: 2 }} title={`Redeemed ${formatPoints(p.redeemed)}`} />
                            <span style={{ fontSize: '0.7rem', color: '#6b7280', whiteSpace: 'nowrap' }}>{formatPoints(p.redeemed)}</span>
                          </div>
                        </div>
                      </div>
                    );
