        ...
        {data && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
              <Kpi label="Creators total" value={data.creators_total.toLocaleString('en-NG')} sub={`${data.creators_verified.toLocaleString('en-NG')} verified`} accent="#340075" />
              <Kpi label="Pending verification" value={data.creators_pending_verification.toLocaleString('en-NG')} accent={data.creators_pending_verification > 0 ? '#9a3412' : undefined} />
              ...
              <Kpi label="Gross creator earnings (30d)" value={formatNaira(data.gross_creator_earnings_30d_kobo)} accent="#15803d" />
              <Kpi label="Platform fee (30d)" value={formatNaira(data.platform_fee_30d_kobo)} sub={`Take rate ${pct(data.take_rate)}`} />
              <Kpi label="Payout liability" value={formatNaira(data.payout_liability_kobo)} sub="Earnings owed, not yet paid" accent={data.payout_liability_kobo > 0 ? '#9a3412' : undefined} />
              <Kpi label="Payouts on KYC hold" value={data.payouts_kyc_hold.toLocaleString('en-NG')} sub={`${formatNaira(data.payouts_kyc_hold_kobo)} · NL-10 gate`} accent={data.payouts_kyc_hold > 0 ? '#9a3412' : undefined} />
              <Kpi label="Content pending moderation" value={data.content_pending_moderation.toLocaleString('en-NG')} accent={data.content_pending_moderation > 0 ? '#9a3412' : undefined} />
              <Kpi label="Content flagged (NL-11)" value={data.content_flagged_open.toLocaleString('en-NG')} accent={data.content_flagged_open > 0 ? '#b91c1c' : undefined} />
              <Kpi label="Billing failures open" value={data.billing_failed_open.toLocaleString('en-NG')} accent={data.billing_failed_open > 0 ? '#9a3412' : undefined} />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {data.top_creators.map((c) => (
                    <tr key={c.id}>
                      <td style={td()}>{c.handle_masked}<div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>{c.id}</div></td>
            ...
            <Card title="Earnings mix (14d)" right={<span style={{ fontSize: '0.75rem', color: '#6b7280' }}>tips (purple) · subs (green) · gated (blue)</span>}>
              {data.earnings_trend.length === 0 ? <p style={{ color: '#6b7280' }}>No data.</p> : (
                  ...
                  {data.earnings_trend.map((p) => {
                    ...
                    return (
                      <div key={p.date} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        ...
                        <div style={{ flex: 1, display: 'flex', height: 12, borderRadius: 3, overflow: 'hidden', background: '#f3f4f6' }}>
                          <div style={{ width: `${w(p.tips_kobo)}%`, background: '#7c3aed' }} title={`Tips ${formatNaira(p.tips_kobo)}`} />
                          <div style={{ width: `${w(p.subs_kobo)}%`, background: '#15803d' }} title={`Subs ${formatNaira(p.subs_kobo)}`} />
                          <div style={{ width: `${w(p.gated_kobo)}%`, background: '#1d4ed8' }} title={`Gated ${formatNaira(p.gated_kobo)}`} />
                        </div>
                        <span style={{ width: 110, textAlign: 'right', fontSize: '0.7rem', color: '#6b7280', whiteSpace: 'nowrap' }}>{formatNaira(total)}</span>
                      </div>
                    );
                  })}
                </div>
