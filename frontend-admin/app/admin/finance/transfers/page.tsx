...
function HealthStrip({ health }: { health: ProviderHealth[] }) {
  return (
    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
      {health.map((h) => (
        <div key={h.provider} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', border: '1px solid #e5e7eb', borderRadius: '0.5rem', padding: '0.5rem 0.9rem', background: '#fff' }}>
          <span style={{ width: 9, height: 9, borderRadius: 9999, background: h.healthy ? '#16a34a' : '#dc2626', display: 'inline-block' }} />
          ...
          <span style={{ fontSize: '0.72rem', color: '#9ca3af' }}>checked {timeAgo(h.last_checked)}</span>
        </div>
      ))}
    </div>
  );
...
export default function TransfersAdminPage() {
  ...
  return (
    <div style={{ padding: '0.5rem 0.5rem 2rem' }}>
      ...
      <Card>
        {loading ? (
          <p style={{ color: '#6b7280' }}>Loading transfers…</p>
        ) : visible.length === 0 ? (
          <p style={{ color: '#6b7280' }}>No transfers in this filter.</p>
        ) : (
                ...
                {visible.map((t) => (
                  <tr key={t.id} style={{ cursor: 'pointer' }}>
                    ...
                    <td style={td()}>
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>{t.account_name}</div>
                    </td>
                    <td style={td()}>{formatKobo(t.amount_kobo)}</td>
                    <td style={td()}>{formatKobo(t.fee_kobo)}</td>
                    <td style={td()}><StatusBadge status={t.status} /></td>
                    <td style={td()}>
                      <Link href={`/admin/finance/transfers/${t.id}`} style={{ color: '#1d4ed8', textDecoration: 'none', fontFamily: 'monospace', fontSize: '0.8rem' }}>
                        {t.reference} →
                      </Link>
                    </td>
                  </tr>
