          ...
          {posture ? (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
                <Kpi label="Model-A-only posture" value={posture.model_a_only ? 'Holding' : 'BREACHED'} accent={posture.model_a_only ? '#15803d' : '#b91c1c'} />
                <Kpi label="BNPL rail repurposed?" value={posture.bnpl_rail_repurposed ? 'YES — factoring risk' : 'No'} accent={posture.bnpl_rail_repurposed ? '#b91c1c' : '#15803d'} />
                <Kpi label="Drift signals" value={posture.drift_signals.length.toString()} accent={critical ? '#b91c1c' : posture.drift_signals.length ? '#9a3412' : '#15803d'} sub={`${critical} critical`} />
                ...
                <div style={{ fontSize: '0.9rem', color: '#374151' }}>
                  ...
                  <p style={{ margin: 0 }}>
                    {posture.model_a_only && !posture.bnpl_rail_repurposed
                      ? <span style={{ color: '#15803d' }}>✔ Posture compliant — collections-only, no fee-fronting detected.</span>
                      : <span style={{ color: '#b91c1c' }}>✖ Posture at risk — review the drift signals below before any further installment rollout.</span>}
                  </p>
                </div>
              </Card>
              ...
              <Card title="Factoring-drift signals">
                {posture.drift_signals.length === 0 ? (
                  <p style={{ color: '#15803d' }}>No drift detected. All installment structures are guardian-pays-school-over-time.</p>
                ) : (
                  <div style={{ display: 'grid', gap: '0.6rem' }}>
                    {posture.drift_signals.map((d) => (
                      <div key={d.id} style={{ border: `1px solid ${d.severity === 'critical' ? '#fecaca' : '#fde68a'}`, background: d.severity === 'critical' ? '#fef2f2' : '#fffbeb', borderRadius: '0.5rem', padding: '0.75rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem', flexWrap: 'wrap' }}>
                          <strong>{d.school_name}</strong>
                          <span style={{ display: 'flex', gap: '0.4rem' }}><Badge status={d.severity} /><span style={{ fontSize: '0.74rem', color: '#6b7280' }}>{timeAgo(d.detected_at)}</span></span>
                        </div>
                        <p style={{ margin: '0.4rem 0 0', fontSize: '0.86rem', color: '#374151' }}>{d.signal}</p>
                      </div>
                    ))}
                  </div>
                )}
