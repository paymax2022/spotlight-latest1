...
export default function LabPayoutsPage() {
  ...
  return (
    <div style={{ padding: '0.5rem 0.5rem 2rem' }}>
      <PageHeader title="Lab payouts" subtitle="Settle released-to-provider funds (HL-9) to verified labs. KYC-tier + AML gated — payouts are fail-closed until checks clear (HL-10). Amounts in ₦." action={<button onClick={load} style={btn()}>Refresh</button>} />
      ...
      <FilterBar>
        ...
        <div style={{ minWidth: 240 }}>
          ...
          <input style={input()} placeholder="Lab, id…" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load()} />
        </div>
      ...
      <Card>
        <StateBlock loading={loading} error={error} empty={rows.length === 0} emptyText="No payouts match.">
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              <th style={th()}>Lab</th><th style={th()}>KYC</th><th style={th()}>AML</th>
              <th style={th()}>Collected</th><th style={th()}>Fees</th><th style={th()}>Net payable</th>
              <th style={th()}>Created</th><th style={th()}>Status</th><th style={th()}>Actions</th>
            </tr></thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  <td style={td()}><strong>{r.lab_masked}</strong><div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>{r.id}</div></td>
