...
function statusColor(status: string): string {
  ...
  if (/(reject|fail|block|suspend|invalid|controlled|blocked)/.test(v)) return colors.danger;
  if (/(pending|warn|flag|held|hold|recall)/.test(v)) return colors.warning;
  if (/(approve|verified|active|complete|ok|dispensed|fulfilled)/.test(v)) return colors.success;
  return colors.info;
}
