...
import { MarketplaceTabs, Kpi, DisclosureNote } from './_ui';
  ...
  return (
    <Page>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
        <Kpi label="Listings pending review" value={pendingListings != null ? String(pendingListings) : '—'} accent={pendingListings && pendingListings > 0 ? colors.warning : undefined} />
        <Kpi label="Open safety flags" value={openFlags != null ? String(openFlags) : '—'} accent={openFlags && openFlags > 0 ? colors.danger : undefined} />
        <Kpi label="Active boosts" value={activeBoosts != null ? String(activeBoosts) : '—'} accent={activeBoosts && activeBoosts > 0 ? colors.success : undefined} />
      </div>

      <Card title="Modules">
        <ul style={{ margin: 0, paddingLeft: '1.1rem', fontSize: '0.88rem', color: colors.muted, lineHeight: 1.9 }}>
