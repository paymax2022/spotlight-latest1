...
function decisionsFor(status: string): { value: AmbassadorDecision; label: string; variant: 'primary' | 'danger' | 'outline' }[] {
  switch (status) {
    case 'applied':
      return [
      ...
      {error && (
        <Card style={{ marginBottom: 16, borderLeft: `3px solid ${colors.danger}` }}>
          ...
          <div style={{ fontSize: '0.8rem', color: colors.muted, marginTop: 10 }}>
            Deciding an application needs the <code>referral.amb.manage</code> permission; viewing needs <code>referral.amb.view</code>.
          </div>
        </Card>
      )}
