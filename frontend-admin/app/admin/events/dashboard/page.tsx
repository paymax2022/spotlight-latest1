        ...
        {data && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
              <Kpi label="GMV today" value={formatNaira(data.gmv_today_kobo)} sub={`${data.tickets_sold_today.toLocaleString('en-NG')} tickets today`} accent={colors.primary} />
              ...
              <Kpi label="Cashless float" value={formatNaira(data.cashless_float_kobo)} sub="Loaded on event wallets (closed-loop)" accent={colors.info} />
              <Kpi label="Cashless liability" value={formatNaira(data.cashless_liability_kobo)} sub="Unspent — owed back to customers" accent={data.cashless_liability_kobo > 0 ? colors.warning : undefined} />
              <Kpi label="Residual refund pending" value={formatNaira(data.residual_refund_pending_kobo)} sub="NL-3 watch — refund at close" accent={data.residual_refund_pending_kobo > 0 ? colors.danger : undefined} />
              ...
              <Kpi label="Pending approval" value={data.events_pending_approval.toLocaleString('en-NG')} accent={data.events_pending_approval > 0 ? colors.warning : undefined} />
              <Kpi label="Vendors active" value={data.vendors_active.toLocaleString('en-NG')} />
              <Kpi label="Payouts on KYC hold" value={data.vendor_payouts_kyc_hold.toLocaleString('en-NG')} sub="NL-10 gate" accent={data.vendor_payouts_kyc_hold > 0 ? colors.warning : undefined} />
              <Kpi label="Settlement breaks" value={data.settlement_breaks_open.toLocaleString('en-NG')} sub={formatNaira(data.settlement_break_value_kobo)} accent={data.settlement_breaks_open > 0 ? colors.danger : undefined} />
