...
DECLARE
  ...
  active_settings_record public.academy_settings%ROWTYPE;
BEGIN
  SELECT *
  ...
  IF FOUND
     AND active_settings_record.registration_type = 'paid'
     AND COALESCE(active_settings_record.application_fee, 0) > 0
     AND p_amount_paid < active_settings_record.application_fee THEN
