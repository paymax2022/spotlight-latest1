...
export default function GateRepairsScreen() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['repairs', estateId, 'mine', 'gate'],
    queryFn: async () => {
      if (!estateId) return [];
      const res = await fetch(`/api/estates/${estateId}/repairs?scope=mine&category=gate`);
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <Pressable style={[s.row, !isLast && s.rowBorder]} onPress={() => router.push(`/repairs/${item.id}` as never)}>
        <View style={s.catBox}><Ionicons name="lock-closed-outline" size={22} color="#dc2626" /></View>
        <View style={{ flex: 1 }}>
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.header}>
        ...
        <Text style={s.hTitle}>Gate Repairs</Text>
        <View style={{ width: 38 }} />
      </View>
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              <Ionicons name="lock-closed-outline" size={48} color={colors.neutral.placeholder} />
              <Text style={s.emptyTxt}>No gate repair tickets</Text>
            </View>
          }
        />
      )}
...
const s = StyleSheet.create({
  ...
  catBox: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: '#dc262622' },
