...
const CATEGORIES = ['Estate', 'Meeting', 'Repair', 'General'] as const;
...
export default function CreateTask() {
  ...
  const mut = useMutation({
    mutationFn: () => createTask(estateId, { title, description, assignee_name: assignee, due_date: dueDate, priority, estate_id: estateId }),
    onSuccess: () => {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Pressable style={[styles.primaryBtn, mut.isPending && { opacity: 0.6 }]} onPress={() => mut.mutate()} disabled={mut.isPending || !title.trim()}>
          <Text style={styles.primaryBtnText}>{mut.isPending ? 'Creating...' : 'Create Task'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  chipRowWrap: { flexDirection: 'row', gap: 10, flexWrap: 'wrap' },
