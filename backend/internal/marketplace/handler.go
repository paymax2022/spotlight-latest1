...
// CreateOrder POST /orders (Idempotency-Key)
func (h *Handler) CreateOrder(c *gin.Context) {
	...
	o, err := h.svc.CreateOrder(c.Request.Context(), uid, idemKeyOf(c), in)
	if err != nil {
		fail(c, err)
		return
	}
	respond(c, http.StatusCreated, orderView(o))
}

// GetOrder GET /orders/:id — OLA: buyer or seller.
func (h *Handler) GetOrder(c *gin.Context) {
	uid, ok := requireUser(c)
	if !ok {
		return
	}
	o, err := h.svc.repo.GetOrder(c.Request.Context(), c.Param("id"))
	if err != nil {
		fail(c, err)
		return
	}
	if uid != o.BuyerID && uid != o.SellerID {
		fail(c, ErrForbidden)
		return
	}
...
// ListOrders GET /orders?role=buyer|seller&status=
func (h *Handler) ListOrders(c *gin.Context) {
	...
	limit, offset := pageParams(c)
	os, err := h.svc.repo.ListOrders(c.Request.Context(), uid, role, c.Query("status"), limit, offset)
	if err != nil {
		fail(c, err)
		return
	}
	respond(c, http.StatusOK, os)
}

// FundOrder POST /orders/:id/fund (Idempotency-Key)
func (h *Handler) FundOrder(c *gin.Context) {
	uid, ok := requireUser(c)
	if !ok {
		return
	}
	...
	o, err := h.svc.FundOrder(c.Request.Context(), c.Param("id"), uid, idemKeyOf(c), in)
	if err != nil {
		fail(c, err)
		return
	}
...
// AcceptOrder POST /orders/:id/accept (seller)
func (h *Handler) AcceptOrder(c *gin.Context) {
	uid, ok := requireUser(c)
	if !ok {
		return
	}
	o, err := h.svc.SellerAccept(c.Request.Context(), c.Param("id"), uid)
	if err != nil {
		fail(c, err)
		return
	}
...
// ConfirmDelivery POST /orders/:id/confirm-delivery (buyer release)
func (h *Handler) ConfirmDelivery(c *gin.Context) {
	uid, ok := requireUser(c)
	if !ok {
		return
	}
	o, err := h.svc.ConfirmDelivery(c.Request.Context(), c.Param("id"), uid)
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) CancelOrder(c *gin.Context) {
	...
	o, err := h.svc.CancelOrder(c.Request.Context(), c.Param("id"), uid)
	if err != nil {
		fail(c, err)
		return
	}
...
// OpenDispute POST /orders/:id/dispute (Idempotency-Key)
func (h *Handler) OpenDispute(c *gin.Context) {
	...
	d, err := h.svc.OpenDispute(c.Request.Context(), c.Param("id"), uid, idemKeyOf(c), in)
	if err != nil {
		fail(c, err)
		return
	}
	respond(c, http.StatusCreated, d)
}

// SubmitReview POST /orders/:id/review
func (h *Handler) SubmitReview(c *gin.Context) {
	...
	rev, err := h.svc.SubmitReview(c.Request.Context(), uid, c.Param("id"), body.Rating, body.Comment)
	if err != nil {
		fail(c, err)
		return
	}
	respond(c, http.StatusCreated, rev)
}

// ─── Disputes (member reads) ─────────────────────────────────────────────────

// GetDispute GET /disputes/:id — OLA: a party to the order.
func (h *Handler) GetDispute(c *gin.Context) {
	...
	o, oerr := h.svc.repo.GetOrder(c.Request.Context(), d.OrderID)
	if oerr != nil {
		fail(c, oerr)
		return
	}
...
// SubmitEvidence POST /disputes/:id/evidence
func (h *Handler) SubmitEvidence(c *gin.Context) {
	...
	if err := h.svc.SubmitEvidence(c.Request.Context(), uid, c.Param("id"), body.Type, body.URLOrText); err != nil {
		fail(c, err)
		return
	}
...
// AppealDispute POST /disputes/:id/appeal
func (h *Handler) AppealDispute(c *gin.Context) {
	uid, ok := requireUser(c)
	if !ok {
		return
	}
	d, err := h.svc.AppealDispute(c.Request.Context(), uid, c.Param("id"))
	if err != nil {
		fail(c, err)
		return
	}
...
// orderView adds the computed total_payable_kobo to the wire shape (§3.1).
func orderView(o *Order) gin.H {
	return gin.H{
		...
		"escrow_fee_kobo":     o.EscrowFeeKobo,
		"delivery_fee_kobo":   o.DeliveryFeeKobo,
		"total_payable_kobo":  o.TotalPayableKobo(),
		"status":              o.Status,
		"ledger_fund_ref":     o.LedgerFundRef,
		"ledger_release_ref":  o.LedgerReleaseRef,
		"delivery_ref":        o.DeliveryRef,
		"inspection_deadline": o.InspectionDeadline,
