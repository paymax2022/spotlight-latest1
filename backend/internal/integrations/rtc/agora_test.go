...
// TestAgoraKnownAnswer is the certification against Agora's official builder.
// It is SKIPPED until the vector is filled: run the official
...
// wantToken. Once filled, this asserts our builder produces content identical
// to Agora's. (Until then, TestAgoraTokenRoundTrip guards the wire format +
// signing spec.)
//
// This compares the INFLATED payload, not the raw compressed bytes: zlib
// implementations are free to encode an equivalent stream differently at the
// final empty block (e.g. Go's compress/zlib Close() emits a fixed-Huffman
// terminator, "03 00", while the reference vector below — as produced by the
// official AgoraIO/Tools builder — emits a stored-block Z_SYNC_FLUSH-style
// terminator, "01 00 00 ff ff"). Both are valid DEFLATE streams that inflate
// to byte-identical content with the same Adler-32 checksum; asserting on the
// raw bytes made this test fail on a real Go build despite the token being
// wire-compatible and correctly signed.
func TestAgoraKnownAnswer(t *testing.T) {
	...
	// Content-identical to our buildAgoraRTCTokenAt — certifies wire format parity.
	...
	inflateToken := func(tok string) []byte {
		t.Helper()
		if !strings.HasPrefix(tok, "007") {
			t.Fatalf("missing 007 version prefix: %q", tok[:min(6, len(tok))])
		}
	...
	gotPayload := inflateToken(got)
	wantPayload := inflateToken(wantToken)
	if !bytes.Equal(gotPayload, wantPayload) {
		t.Errorf("inflated token content mismatch vs official builder:\n got=%x\nwant=%x", gotPayload, wantPayload)
	}
}
