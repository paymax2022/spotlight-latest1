...
export default function StockSellReviewScreen() {
  ...
  if (asset.isError || !asset.data) {
    return (
      <SafeAreaView style={styles.safe} edges={['top']}>
        <ScreenHeader title="Review order" />
        <StateView kind="error" title="Couldn't prepare your order" message="Please go back and try again." actionLabel="Back" onAction={() => goBack('/stocks/sell')} />
      </SafeAreaView>
    );
  }
  ...
  const confirm = () => {
    ...
    router.push({
      pathname: '/stocks/sell/processing',
      params: {
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.hero}>
          ...
          <Text style={styles.heroAmount}>{`Sell ${formatShares(quantity)} of ${a.symbol}`}</Text>
      ...
      <SafeAreaView edges={['bottom']} style={styles.footer}>
        <PrimaryButton label={`Confirm sell · ${formatMoneyObj(estimate.total)}`} onPress={confirm} />
      </SafeAreaView>
    </SafeAreaView>
  );
}
