...
export default function StockSellSuccessScreen() {
  ...
  const title = filled ? 'Sale filled 🎉' : 'Sale placed — pending';
  const sub = filled
    ? `You sold ${formatShares(Number(p.quantity))} of ${p.symbol} for ${formatMoney(Number(p.total), currency)}. Proceeds are available once the trade settles.`
    : `Your order to sell ${formatShares(Number(p.quantity))} of ${p.symbol} is working. We'll credit your cash once it fills and settles.`;
