...
import { useDocuments } from '@/features/fractionalre/hooks';
...
export default function DocumentsVault() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Documents vault" />
      {docs.isLoading ? (
        <StateView kind="loading" message="Loading documents…" />
      ) : (docs.data?.length ?? 0) === 0 ? (
        <StateView kind="empty" title="No documents yet" message="Certificates, statements and signed disclosures appear here." icon="FileText" />
      ) : (
        <FlatList
          data={docs.data}
          keyExtractor={(d) => d.id}
          ...
          renderItem={({ item }) => (
            <Pressable style={styles.row} onPress={() => Linking.openURL(item.url).catch(() => {})}>
              ...
              <View style={styles.text}>
                ...
                <Text style={styles.sub}>{item.offeringTitle} · {relativeDate(item.issuedAt)} · {item.sizeKb} KB</Text>
              </View>
