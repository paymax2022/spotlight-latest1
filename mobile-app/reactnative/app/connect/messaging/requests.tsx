...
// MS-03 — Message-request inbox. Date-mode "wants to message you" requests.
// Pending requests are NOT threads (§5); accepting one creates the thread.
// Connect-type requests are handled by connection-requests.tsx.
    ...
    <View style={styles.card}>
      <View style={styles.cardTop}>
        ...
        <View style={styles.cardBody}>
          ...
          <Text style={styles.sub}>Wants to message you · {relativeTime(request.createdAt)}</Text>
        </View>
      </View>
...
export default function MessageRequests() {
  ...
  const messageRequests = (data ?? []).filter((r) => r.kind === 'message');

  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Message requests" />
      ...
      {isLoading ? (
        ...
        <StateView
          ...
          message="When someone asks to message you, it will appear here."
        />
      ) : (
        <FlatList
