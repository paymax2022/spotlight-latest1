...
export default function AdminMemberList() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Manage members" />
      ...
      {members.isLoading ? (
        ...
        <StateView kind="empty" icon="Users" title="No members found" message={search ? `Nothing matches “${search}”.` : 'No members match this filter.'} />
      ) : (
        <FlatList
          data={members.data ?? []}
          keyExtractor={(m) => m.id}
          ...
          renderItem={({ item }) => (
            <MemberRow member={item} onPress={() => router.push(`/association/admin/members/${item.id}`)} />
          )}
        />
      )}
    </SafeAreaView>
