...
import type { ConsultationPricing } from '@/types/doctor.batch1';
...
export default function VetPricingScreen() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      router.push('/(doctor)/vet/profile/setup/preview');
    } catch { /* surfaced */ }
  };
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <WizardProgress current={9} total={10} label="Consultation pricing" />
          ...
          <SectionCard title="Availability" style={styles.card}>
            ...
            <Pressable style={styles.link} onPress={() => router.push('/(doctor)/availability')} accessibilityRole="button" accessibilityLabel="Set availability schedule">
              <CalendarClock size={18} color={Colors.secondary} strokeWidth={2} />
              <Text style={styles.linkText}>Set your availability schedule</Text>
            </Pressable>
          </SectionCard>

          <PrimaryButton label="Continue to preview" onPress={handleNext} loading={save.isPending} disabled={!canSubmit} style={styles.btn} />
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  link:       { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, marginTop: Spacing.sm, paddingTop: Spacing.sm, borderTopWidth: 1, borderTopColor: Colors.surfaceContainerHigh },
  linkText:   { ...Typography.labelMd, color: Colors.secondary },
