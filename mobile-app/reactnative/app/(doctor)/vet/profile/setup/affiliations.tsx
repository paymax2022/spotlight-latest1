...
import type { ClinicAffiliation } from '@/types/doctor.batch1';

const blank = (): ClinicAffiliation => ({ id: `aff-${Date.now()}`, name: '', role: '', state: undefined, city: '', isPrimary: false });

export default function VetAffiliationsScreen() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      router.push('/(doctor)/vet/profile/setup/experience');
    } catch { /* surfaced */ }
  };
  ...
  if (isLoading && !draft) {
    return (
      <SafeAreaView style={styles.safe} edges={['top']}>
        <TeleHeader title="Clinic affiliations" />
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <WizardProgress current={7} total={10} label="Clinic affiliation" />

          <SectionCard title="Where you practise" style={styles.card}>
            <Text style={styles.hint}>Add the clinics or animal hospitals you are affiliated with.</Text>
            ...
            {editing ? (
              <View style={styles.form}>
                <TextInputField label="Clinic / hospital name" placeholder="e.g. Greenfield Vet Clinic" value={editing.name} onChangeText={(name) => set({ name })} />
                <TextInputField label="Your role" placeholder="e.g. Consultant Veterinarian" value={editing.role ?? ''} onChangeText={(role) => set({ role })} />
