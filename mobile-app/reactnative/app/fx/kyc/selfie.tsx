...
export default function KycSelfieScreen() {
  const [phase, setPhase] = useState<'idle' | 'scanning' | 'done'>(kycDraft.current.identity.selfieUploaded ? 'done' : 'idle');

  const capture = () => {
    setPhase('scanning');
    setTimeout(() => {
      kycDraft.current.identity.selfieUploaded = true;
      setPhase('done');
    }, 1600);
  };

  const next = () => {
    if (kycDraft.current.accountType === 'business') router.push('/fx/kyc/business');
    else router.push('/fx/kyc/submitted');
  };

  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Liveness check" subtitle="Step 4 of 4" />
      ...
      <View style={styles.center}>
        <View style={[styles.ring, phase === 'done' && styles.ringDone]}>
          {phase === 'scanning'
            ...
            : phase === 'done'
            ? <CheckCircle2 size={64} color={Colors.tertiaryContainer} strokeWidth={1.8} />
