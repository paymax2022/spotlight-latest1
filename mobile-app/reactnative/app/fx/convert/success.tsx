...
export default function ConvertSuccessScreen() {
  const p = useLocalSearchParams<{ reference: string; from: string; to: string; source: string; dest: string; txId: string }>();
  const from = p.from as CurrencyCode;
  const to = p.to as CurrencyCode;
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.center}>
        ...
        <Text style={styles.sub}>
          {formatMoney(Number(p.source), from)} converted to {formatMoney(Number(p.dest), to)} at the locked rate.
        </Text>
      ...
      <SafeAreaView edges={['bottom']} style={styles.footer}>
        <Pressable
          ...
          onPress={() => router.replace(`/fx/transactions/${p.txId}`)}
