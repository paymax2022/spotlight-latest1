...
export default function PastTrips() {
  const trips = useTrips('past');
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Past stays" subtitle="Completed stays unlock reviews" />
      {trips.isLoading ? (
        <StateView kind="loading" message="Loading past stays…" />
      ) : trips.isError ? (
        <StateView kind="error" title="Couldn't load" actionLabel="Retry" onAction={() => trips.refetch()} />
      ) : (trips.data?.length ?? 0) === 0 ? (
        <StateView kind="empty" icon="History" title="No past stays yet" message="Completed stays show here." actionLabel="Browse stays" onAction={() => router.replace('/stays')} />
      ) : (
        <FlatList
          data={trips.data}
          keyExtractor={(t) => t.id}
