...
export default function UpcomingTrips() {
  const trips = useTrips('upcoming');
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Upcoming stays" />
      {trips.isLoading ? (
        <StateView kind="loading" message="Loading upcoming stays…" />
      ) : trips.isError ? (
        <StateView kind="error" title="Couldn't load" actionLabel="Retry" onAction={() => trips.refetch()} />
      ) : (trips.data?.length ?? 0) === 0 ? (
        <StateView kind="empty" icon="BedDouble" title="No upcoming stays" message="Book a stay and it will appear here." actionLabel="Browse stays" onAction={() => router.replace('/stays')} />
      ) : (
        <FlatList
          data={trips.data}
          keyExtractor={(t) => t.id}
