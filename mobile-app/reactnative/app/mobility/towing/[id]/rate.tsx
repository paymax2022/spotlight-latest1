...
import { useTowingJob, useRateTowing } from '@/features/mobility/hooks/useModes';
import { clearMockActiveTowing } from '@/features/mobility/api/towing.api';
import { formatNairaWhole } from '@/features/mobility/utils/mobilityFormatters';

export default function RateTowingScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const job = useTowingJob(id);
  const rate = useRateTowing();
  const [stars, setStars] = useState(0);
  const [comment, setComment] = useState('');
  const j = job.data;

  const finish = () => { clearMockActiveTowing(); router.replace('/mobility'); };
  ...
  if (job.isError || !j) return <SafeAreaView style={styles.safe} edges={['top']}><MobilityEdgeState kind={errKind(job.error)} actionLabel="Retry" onAction={() => job.refetch()} /></SafeAreaView>;
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.confirmation}>
          ...
          <Text style={styles.confTitle}>Service complete</Text>
          <Text style={styles.confSub}>{formatNairaWhole(j.fareKobo)} paid from your wallet</Text>
        </View>
        <FareBreakdownCard title="Receipt" fareKobo={j.fareKobo} rows={[{ label: 'Status', valueText: 'Paid' }]} />
        {j.operator && (
          <View style={[styles.card, shadow1]}>
            <Text style={styles.cardTitle}>Rate {j.operator.name}</Text>
