...
export default function ProviderConsultScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title="Patient consult" subtitle="Ada Obi" />

      <View style={styles.banner}>
        <ShieldCheck size={13} color={Colors.teal} strokeWidth={2} />
        <Text style={styles.bannerText}>You are responding as a licensed pharmacist.</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
          <FlatList
            ...
            renderItem={({ item }) => (
              // Provider perspective: pharmacist messages on the RIGHT (outgoing),
              // patient messages on the LEFT (incoming).
              <View style={[styles.bubbleRow, item.fromPharmacist ? styles.right : styles.left]}>
                <View style={[styles.bubble, item.fromPharmacist ? styles.bubbleOut : styles.bubbleIn]}>
            ...
            ListEmptyComponent={<StateView kind="empty" compact icon="MessageCircle" title="No messages yet" message="Reply to start the consult." />}
          />
