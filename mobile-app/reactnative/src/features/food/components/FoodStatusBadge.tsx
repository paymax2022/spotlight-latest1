...
import { STATUS_LABEL, STATUS_TONE, type StatusTone } from '../utils';
...
export default function FoodStatusBadge({ status, label }: { status: OrderStatus; label?: string }) {
  const tone = STATUS_TONE[status];
  return (
    <View style={[styles.badge, { backgroundColor: TONE_BG[tone] }]}>
      <Text style={[styles.text, { color: TONE_FG[tone] }]}>{label ?? STATUS_LABEL[status]}</Text>
    </View>
  );
}
