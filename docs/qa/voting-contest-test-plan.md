...
| AD-006 | Contestant management (approve/reject/reprocess image/DQ) | RBAC | P0 | Pending | Manage | Correct states; re-pipeline; audited | Y | 🚧 Partial | `/admin/contestants` (`frontend-admin`) is still a **scaffold stub** (mock) for the new voting engine. Approve/reject-with-reason is now real: `frontend-admin/app/admin/registration/page.tsx` lists pending applications per contest and calls the tested review endpoint (`POST /api/admin/registration/applications/[id]/review`, RG-006, `applications:review` RBAC). Component-level coverage: `frontend-admin/app/admin/registration/page.test.tsx`. Still no reprocess-image or DQ action for the new engine. |
