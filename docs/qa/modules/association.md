...
| Commission recording → Community/Group Membership earning-row only (no ledger re-post) | INT | `tests/association/commission_recording_test.go` `TestLiveDB_PayInvoice_RecordsCommissionEarningRowOnly` | AUTOMATED (gated) |
| Flag-off (`FeatureAssociationsEnabled=false`) → routes unmounted (404) | SEC | Live-verified (2026-09-16): a real throwaway backend built with `FEATURE_ASSOCIATIONS_ENABLED` unset logged zero `associations` route registrations, and both `GET /api/finance/associations` and `POST /api/finance/associations/dues/x/pay` returned a bare Gin `404 page not found` (route never registered, not an auth 401/403). **Not committed as an automated Go test** — the mounting call lives inside `finance_routes.go`'s single monolithic registration function alongside dozens of unrelated modules with a large real dependency graph (Redis, R2, payment providers, …); building an in-process route-construction harness for this one flag would need either an invasive extraction of that function or a full app boot, neither justified by this one assertion. Re-verify live if the mounting code around L1063 changes. | LIVE-VERIFIED (not automated — see note) |
| HTTP handler layer (`statusFor` mapping, `Idempotency-Key` header wiring) | CON | `internal/association/handler_test.go` `TestStatusFor_MapsErrorsToHTTPStatus` — table-tests every mapped error including a **wrapped** error (`fmt.Errorf("...: %w", err)`, the actual shape every service method returns) to pin that `statusFor` uses `errors.Is`, not `==`; a naive rewrite to direct comparison would silently 500 every real service error. `Idempotency-Key` header→`req.IdempotencyKey` wiring is money-path (`PayInvoice`) and continues to be proven via the existing live-DB tests calling the real HTTP-adjacent flow, not a mocked handler — this module's own standing discipline is DB-executed proof over mocks for money paths. | AUTOMATED (statusFor) / LIVE-VERIFIED (header wiring, via existing PayInvoice live-DB tests) |
...
- `backend/tests/association/commission_recording_test.go` — assert a settled dues pay records exactly ONE `commission_earnings` Community/Group Membership row equal to the platform share and posts NO second ledger entry (the `finance_routes.go` L858-861 contract). Follow the transcribed-invariant style of `money_invariants_test.go`. **DONE** (2026-09-16) — `TestLiveDB_PayInvoice_RecordsCommissionEarningRowOnly`.
- `backend/tests/association/flag_off_test.go` — build the finance router with `FeatureAssociationsEnabled=false` and assert `/associations/*` returns 404 (routes unregistered). **Deliberately not built as a Go test** (2026-09-16) — live-verified instead against a real throwaway backend (zero route registrations logged, bare Gin 404 on both a GET and a POST). See §3's row for the reasoning; revisit only if `finance_routes.go`'s registration function is ever refactored to be testable in isolation.
- `backend/internal/association/handler_test.go` — thin HTTP-layer table test covering `statusFor` mapping. **DONE** (2026-09-16) — `TestStatusFor_MapsErrorsToHTTPStatus`, including a wrapped-error case pinning `errors.Is` usage. `Idempotency-Key` header wiring and `ShouldBindJSON` 400s remain covered via the existing live-DB money-path tests rather than a separate mocked-handler test, per this module's own live-proof-over-mocks discipline for anything money-adjacent.
- Promote the AI-note negative-authz assertion (`ASSOCIATION-AUTHZ-004`) into a DB-free service test with a fake role store if `requireAssocAdmin` is refactored to an injectable dependency, so it runs without `DATABASE_URL`. **TODO** — not attempted this pass; the live-DB coverage for this case is solid (see §3), so this is a nice-to-have for CI speed, not a coverage gap.
