...
**Risk tier:** 1 &nbsp;·&nbsp; **Money-path:** no *for the Go admin control plane specifically* — it posts NO ledger entries; moderation & verification are status-column updates only, and its payments/escrow views are read-only kobo projections. **The member-facing Supabase money plane is a real money path** (see §1 and §6 — `realtor_pay_invoice` posts a ledger debit via `debitWallet`/`debit_wallet_atomic` before it finalizes anything). &nbsp;·&nbsp; **Feature flag:** `FeatureRealtorEnabled` (env `FEATURE_REALTOR_ENABLED`, default `false`; `backend/internal/config/config.go`) — gates only the Go admin control plane; the Supabase RPC money plane below has no feature flag.
...
Realtor exposes two distinct surfaces. (1) An **admin control plane** under `/api/realtor/admin` — overview dashboard, listing-moderation queue + decision, property-verification queue + decision, and read-only payments/escrow views. Every admin route is fail-closed behind the `realtor.manage` RBAC permission (`middleware.RequirePermission`, applied group-wide after `RequireAuthContext`); grant it to Property-Ops / Trust-&-Safety / Finance / Super-Admin. The mobile/marketplace data plane (listing creation, bookings, leases, payments) is served directly from Supabase RPCs and is **out of scope** here — this Go module is control-plane + one bridge only. (2) The **stays→gate-pass bridge** (cross-cutting flow #4, "the moat flow") under `/api/finance/realtor` — a member-facing endpoint that lazily and idempotently auto-issues an estate visitor gate pass for a confirmed shortlet/hotel booking whose unit physically sits inside a managed estate, reusing the estate pass-issuance seam. This surface uses `mapsAuth()` + a **non-blocking** `estate.manage` probe (`estateStaffProbe`), not `realtor.manage`. All monetary values are BIGINT minor units (kobo); the control plane never mutates them. Cross-cutting: `../cross-cutting/authentication.md`, `../cross-cutting/rbac-and-permissions.md`, `../cross-cutting/feature-flags-and-audit.md` (money-invariants apply only weakly to the Go control plane — no ledger writes there).

**⚠️ Scoping correction (Blocker fix, 2026-09-17):** the "no money path" / "read-only kobo projections" language above and in §6 describes the **Go admin control plane ONLY**. It does NOT describe the member-facing Supabase RPC money plane (lease invoice payment: `realtor_pay_invoice` in `supabase/migrations/20260620020000_realtor_backend_rpcs.sql`, called from the mobile app via `mobile-app/reactnative/src/features/realtor/api/realtorLease.api.ts#payInvoice`). That RPC used to finalize a lease payment (mark invoice paid, activate the lease, create a refundable escrow deposit) with **zero verification that any money moved** — no wallet debit, no ledger entry, no Paystack call, reachable in production with no feature flag. It has been fixed (`supabase/migrations/20270220000000_realtor_pay_invoice_require_verified_debit.sql` + `frontend-web/src/server/realtor/invoices.ts` + `frontend-web/app/api/v1/realtor/invoices/[id]/pay/route.ts`): the mobile client now calls that Next.js route, which posts a real, tier-checked, balanced ledger debit via `debitWallet`/`debit_wallet_atomic` **before** calling `realtor_pay_invoice` (now `service_role`-only) to finalize; the RPC itself additionally refuses to finalize a `WALLET` payment unless it finds a matching `ledger_entries` `DEBIT` row for the idempotency key. `PAYSTACK` is refused with 501 (not yet integrated) rather than faked.
...
- **No money path (Go admin control plane only):** the control plane posts NO ledger entries; its payments/escrow views are read-only kobo projections. Money-invariant cases do not apply to it, but assert no admin route can mutate a kobo column. This does **not** apply to the member-facing Supabase RPC money plane — see the scoping correction in §1: `realtor_pay_invoice` now posts a real ledger debit via `debitWallet`/`debit_wallet_atomic` before finalizing, and is locked to `service_role` (`REALTOR-SEC-003`: a direct `authenticated`-role call to `realtor_pay_invoice` must be refused with permission-denied — the exploit path itself, not just the app-layer route, must be closed).
